...
The service exposes the bounded desire-store operations: partition reads, desire writes, and status writes. Envoy removes caller-supplied identity and tenant headers. The gateway validates each remote Applier through a trusted cross-cluster identity mechanism and maps the validated identity to exactly one partition before injecting trusted identity headers. The service derives partition scope exclusively from those injected headers, rejects requested scope that does not match the caller, and never derives scope from client-supplied parameters or untrusted JWT claims.

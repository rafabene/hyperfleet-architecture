...
(`edge` e `edge+api`) os **dois** schemes estão ativos ao mesmo tempo: humanos
sempre usam `Bearer`, e as máquinas (Sentinel, Adapters) usam `ServiceAccount`
quando o gateway está no caminho — é assim que o Authorino escolhe entre OIDC e
TokenReview. Só no modo `api` não há par: sem gateway não há TokenReview, então
**tudo chega como `Bearer`**, inclusive o SA token, validado como JWT contra o
issuer do cluster. Em `edge+api`, nada disso chega à API como veio: o Authorino
reescreve o `Authorization` para `Bearer <wristband>` qualquer que tenha sido o
scheme do cliente.
